...
import (
	...
	"git.liero.se/opentelco/go-swpx/fleet/template"
...
const (
	...
	defaultTemplateCollection string = "templates"
)
...
func New(mc *mongo.Client, database string, logger hclog.Logger) (template.Repository, error) {
	if database == "" {
...
func (r *repo) GetByID(ctx context.Context, id string) (*templatepb.Template, error) {
	// get a device configuration by its ID, this is used to get a specific device configuration
	filter := bson.M{"_id": id}
	var deviceConfiguration templatepb.Template
...
func (r *repo) List(ctx context.Context, params *templatepb.ListParameters) ([]*templatepb.Template, error) {
	...
	var deviceConfigurations []*templatepb.Template
...
func (r *repo) Upsert(ctx context.Context, conf *templatepb.Template) (*templatepb.Template, error) {
